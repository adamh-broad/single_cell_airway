---
title: "Analysis of 'pulse-seq' lineage trace of 65,000 airway epithelial cells"
...
knitr::opts_chunk$set(
  fig.path = "PulseSeq_figs/PulseSeq-"
)
```
...
### Load UMI count data from GEO and clustering and dimensionality reduction (output of 'Pre_process' analysis)
```{r load_data}
## Downloading UMI count data using a terminal: 
> wget ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE103nnn/GSE103354/suppl/GSE103354_PulseSeq_UMI_counts.rds.gz
> gunzip GSE103354_PulseSeq_UMI_counts.rds.gz
## Reading UMI count data from file. Note that processed UMI data already has ~500 contaminating immune (dendritic) cells, and all low quality (<1000 genes detected) cells removed
ps_umis = readRDS("GSE103354_PulseSeq_UMI_counts.rds")
...
### Run PCA, t-SNE
``` {r tsne, fig.height = 4, fig.width = 9, fig.align = "center"}
...
```{r fig3c, fig.height = 6, fig.width = 10, fig.align = "center", eval=F}
...
```{r fig_ED_6c, fig.height = 12, fig.width = 10, fig.align = "center"}
d = as.data.frame.matrix(table(md$batch, md[, use_clust]))
d$timepoint = as.numeric(gsub("Tp", "", str_split_fixed(rownames(d),fixed("_"),3)[,1]))
d$color = str_split_fixed(rownames(d),fixed("_"),3)[,3]
d$batch = paste(str_split_fixed(rownames(d),fixed("_"),3)[,1], str_split_fixed(rownames(d),fixed("_"),3)[,2], sep = "_")
...
```{r fig3d, fig.height = 6, fig.width = 6, fig.align = "center"}
